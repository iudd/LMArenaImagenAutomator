...
async function initConsoleLoginMode(config, options, chromePath, chromeConfig) {
    ...
    // 添加远程服务器友好的状态提示
    logger.info('浏览器', '远程服务器部署提示:');
    logger.info('浏览器', '- 登录状态将自动保存到 data/login-status.json');
    logger.info('浏览器', '- 每30秒输出一次状态检查信息');
    logger.info('浏览器', '- 可通过检查日志文件确认登录状态');
    
    return new Promise((resolve) => {
        let keepRunning = true;
        let statusReportCount = 0;
        
        // 定期状态报告（远程服务器友好）
        const statusReportInterval = setInterval(async () => {
            if (!keepRunning) {
            ...
            try {
                ...
                logger.info('浏览器', `[状态报告 #${statusReportCount}] 程序运行正常`);
                logger.info('浏览器', `当前页面: ${pageTitle} (${currentUrl.substring(0, 50)}...)`);
                
                // 检查浏览器连接状态
                if (browser.isConnected()) {
                    logger.info('浏览器', '浏览器连接状态: 正常');
                } else {
                    logger.warn('浏览器', '浏览器连接状态: 已断开');
                }
                
            } catch (e) {
                logger.warn('浏览器', `状态检查失败: ${e.message}`);
            }
        }, 30000); // 每30秒报告一次状态
        ...
        const handleExit = async () => {
            ...
            try {
                // 保存最终状态
                await saveLoginStatus(page);
                logger.info('浏览器', '最终登录状态已保存');
            } catch (e) {
                logger.warn('浏览器', `保存最终状态失败: ${e.message}`);
            }
        ...
        browser.on('disconnected', () => {
            logger.warn('浏览器', '浏览器已断开连接，程序将退出');
            keepRunning = false;
            clearInterval(statusReportInterval);
            handleExit();
        });
...
async function monitorLoginProcess(page, client, productName) {
    ...
    const checkInterval = setInterval(async () => {
        try {
            ...
            // 每10次检查输出一次状态信息（远程服务器友好）
            if (statusCheckCount % 10 === 0) {
                logger.info('登录助手', `登录监控中... 当前URL: ${currentUrl.substring(0, 50)}...`);
            }
            
            if (currentContent !== lastContent) {
                lastContent = currentContent;
                const hasSuccess = await detectLoginElements(page);
                
                // 如果检测到登录成功且之前未检测到
                if (hasSuccess && !loginSuccessDetected) {
                    loginSuccessDetected = true;
                    logger.info('登录助手', '登录成功！正在保存登录状态...');
                    ...
                    logger.info('登录助手', '登录状态已保存，程序将保持运行');
                    logger.info('登录助手', '您可以通过检查日志文件来确认登录状态');
                }
            }
        } catch (e) {
            // 忽略错误
 ...
 * @param {object} page - 页面对象
 */
async function saveLoginStatus(page) {
    try {
        ...
        const statusFile = path.join(process.cwd(), 'data', 'login-status.json');
        ...
        logger.info('登录助手', `登录状态已保存到: ${statusFile}`);
        
    } catch (e) {
        logger.warn('登录助手', `保存登录状态失败: ${e.message}`);
    }
}
    ...
    try {
        ...
        const successIndicators = await page.evaluate(() => {
            ...
            return {
                ...
                hasLMArenaUI: url.includes('lmarena.ai') && (bodyText.includes('chat') || bodyText.includes('generate')),
                hasUserInterface: document.querySelector('textarea, button, input[type="text"]') !== null && 
                                  !url.includes('accounts.google.com') && !url.includes('login')
            };
        });

        const isSuccess = successIndicators.hasWelcome || 
                         successIndicators.hasDashboard || 
                         successIndicators.hasSuccess ||
                         successIndicators.hasLMArenaUI ||
                         successIndicators.hasUserInterface;
