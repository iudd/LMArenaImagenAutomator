...
function isContainerEnvironment() {
    return process.env.DOCKER_ENV === 'true' || 
           ...
           (process.env.HOME === '/app' && process.env.PWD === '/app');
}
...
function getDefaultConfig() {
    ...
    // 容器环境专用配置
    const chromeConfig = isContainer ? 
        `  # 浏览器可执行文件路径 (容器中使用系统Chrome)
  # 在 Docker 环境中使用系统安装的 Chrome
...
# ${isContainer ? '容器环境检测到，已应用优化配置' : '本地环境配置'}
